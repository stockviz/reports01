...
#industrySummDb <- "/mnt/data/recoll/INDUSTRY_NOTEBOOK.db"

source("/mnt/hollandr/config.r")
industrySummDb <- "/mnt/icelandD/recoll/INDUSTRY_NOTEBOOK.db"
...
aiSummaryAsof <- dbGetQuery(mydb, "SELECT PROMPT, max(AS_OF) MA FROM DISTILLATION WHERE INDUSTRY = $1 group by PROMPT", params=list(iName))
aiSummaries <- tibble()
for(i in 1:nrow(aiSummaryAsof)){
  prompt <- aiSummaryAsof$PROMPT[i]
  asof <- aiSummaryAsof$MA[i]
  aiSummary <- dbGetQuery(mydb, "SELECT PROMPT, SUMMARY, SOURCES, AS_OF FROM DISTILLATION 
                          WHERE INDUSTRY = $1 and PROMPT = $2 and AS_OF = $3
                          order by PROMPT", 
                          params=list(iName, prompt, strftime(asof, "%Y-%m-%d")))
  aiSummaries <- rbind(aiSummaries, aiSummary)
}
dbDisconnect(mydb)
...
if(nrow(aiSummaries) > 0){
  cat("## AI Summaries", "\n\n")
  for(i in 1:nrow(aiSummaries)){
    cat("### ", paste0("**", aiSummaries$PROMPT[i], "**"), "\n\n")
    cat("*asof: ", aiSummaries$AS_OF[i], " *", "\n\n")
    summ <- str_replace_all(aiSummaries$SUMMARY[i], "#### ", "###### ")
    summ <- str_replace_all(summ, "### ", "##### ")
    summ <- str_replace_all(summ, "## ", "#### ")
    summ <- str_replace_all(summ, "# ", "### ")
    
    footnoteFile <- sprintf("footnote/%s.%d.txt", iName, i)
    writeLines(aiSummaries$SOURCES[i], footnoteFile)
    summ <- paste0(summ, "\n\n", sprintf("*[sources](%s) *", footnoteFile))
