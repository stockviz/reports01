...
industryDf <- sqlQuery(lcon, "select a.symbol, a.basic_industry from nse_industry a join (select b.symbol, max(b.time_stamp) mt from nse_industry b group by symbol) t on a.symbol=t.symbol and a.time_stamp = t.mt")
industryCount <- industryDf |> group_by(basic_industry) |> summarise(ctr = n()) |> filter(ctr >= 5)
...
createPlots <- function(){
	...
	#for(i in 1:1){
	for(i in 1:nrow(industryCount)){
		
		iName <- industryCount$basic_industry[i]
		fName <- gsub("[^[:alnum:] ]| ", "", iName)
		
		#calculate equal weight industry returns
		print(iName)
		tryCatch({
		  syms <- industryDf |> filter(basic_industry == iName) |> select(symbol)
		  for(j in 1:nrow(syms)){
		    sym <- syms$symbol[j]
